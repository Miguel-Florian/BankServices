...
// Initiate a deposit and call the api to make a deposit to a specified account number
func MakeDeposit()gin.HandlerFunc{
	return func(c *gin.Context){
		...
		if err != nil{
			...
			fmt.Println(payload)
			url1 := fmt.Sprintf("http://localhost:3000/api/accountservices/account/deposit/%s",params)
			fmt.Println(url1)
			...
			c.JSON(http.StatusOK, responses.Response{Status: http.StatusOK, Message: "Deposit successfully done", Data: map[string]interface{}{"Amount": result}})
...
func MakeWithdraw() gin.HandlerFunc{
	return func(c *gin.Context){
		...
		if err != nil{
			...
			var retrait model.Transaction
		
			if err := c.BindJSON(&retrait); err != nil {
				c.JSON(http.StatusBadRequest, responses.Response{Status: http.StatusBadRequest, Message: "error", Data: map[string]interface{}{"data": err.Error()}})
				return
			}
			//using the library validation to validate required fields
			if validationErr := validate.Struct(&retrait); validationErr != nil {
				c.JSON(http.StatusBadRequest, responses.Response{Status: http.StatusBadRequest, Message: "error", Data: map[string]interface{}{"data": validationErr.Error()}})
				return
			}
			newRetrait := model.Transaction{
				ID : 				primitive.NewObjectID(),
				Number_account: 	retrait.Number_account,
				Amount:				retrait.Amount,
				Date:				time.Now(),
			}
			//fmt.Println(newRetrait)

			var retraitJson model.Transaction
			nd,_ := json.Marshal(newRetrait)
			er := json.Unmarshal(nd, &retraitJson)
			if er != nil {
    			fmt.Println(er)
			}
			//fmt.Println(retraitJson)
			retraitAmount := retraitJson.Amount
			//fmt.Println(depotAmount)
			result, err := withdrawalCollection.InsertOne(ctx, newRetrait)
			if err != nil {
				c.JSON(http.StatusInternalServerError, responses.Response{Status: http.StatusInternalServerError, Message: "error", Data: map[string]interface{}{"data": err.Error()}})
				return
			}
			...
			payload := map[string]int64{
				"amount": lastAmount - retraitAmount,
			}
			// encode payload to JSON
			jsonPayload, err := json.Marshal(payload)
			if err != nil {
			...
			c.JSON(http.StatusOK, responses.Response{Status: http.StatusOK, Message: "Withdraw successfully done", Data: map[string]interface{}{"Amount": result}})
